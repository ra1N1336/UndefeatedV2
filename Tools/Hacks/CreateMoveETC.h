...
public:
	...
	static void BhopMovement( Vector& OrigAng )
	{
		...
		if( Settings.GetSetting( Tab_Misc, Misc_CircleStrafe ) )
		...
		if( !( LocalInfo.Flags & FL_ONGROUND ) )
		{
			if( Hacks.CurrentCmd->buttons & IN_JUMP )
			{
				if( !( LocalInfo.Flags & FL_INWATER ) )
					if( Settings.GetSetting( Tab_Misc, Misc_AutoJump ) )
						Hacks.CurrentCmd->buttons &= ~IN_JUMP;
				if( Settings.GetSetting( Tab_Misc, Misc_AutoStrafe ) )
				{
					...
					else if( Hacks.CurrentCmd->forwardmove == 0 && Hacks.CurrentCmd->sidemove == 0 )
					{
						...
						{
							if( Settings.GetSetting( Tab_Misc, Misc_AutoStraferMode ) == 3 )
							{
							...
							else if( Settings.GetSetting( Tab_Misc, Misc_AutoStraferMode ) == 4 )
							{
						...
						else
						{
							...
							static int zhop = 0;
							static int timer = 0;
							if( Settings.GetSetting( Tab_Misc, Misc_ZStrafe ) )
							{
								if( GetAsyncKeyState( ( int )Settings.GetSetting( Tab_Misc, Misc_ZStrafeKey ) ) )
								{
									if( zhop == 0 )
										zhop = -1;
									if( timer > 15 )
							...
							if( Settings.GetSetting( Tab_Misc, Misc_AutoStraferMode ) != 1 && Settings.GetSetting( Tab_Misc, Misc_AutoStraferMode ) != 0 )
							{
								...
								if( Settings.GetSetting( Tab_Misc, Misc_AutoStraferMode ) == 4 )
								{
									Hacks.CurrentCmd->forwardmove = Hacks.CurrentCmd->sidemove;
									Hacks.CurrentCmd->sidemove = 0;
									Hacks.CurrentCmd->viewangles.y -= 90;
	...
	static void CleanUp( Vector OrigAng )
	{
		...
		if( Settings.GetSetting( Tab_Misc, Misc_Aimstep ) )
